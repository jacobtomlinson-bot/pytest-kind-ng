...
- **test** job: matrix over Python 3.7 (on `ubuntu-22.04`), 3.8, 3.9, 3.10 (on `ubuntu-latest`). Installs uv, runs
  `uv run --locked task test`, and dumps `docker ps --all` on failure.
